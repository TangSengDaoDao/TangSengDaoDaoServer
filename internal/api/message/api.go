...
import (
	...
	commonapi "github.com/TangSengDaoDao/TangSengDaoDaoServer/internal/api/common"
...
func (m *Message) Route(r *wkhttp.WKHttp) {
	...
	replies := r.Group("/v1/replies", m.ctx.AuthMiddleware(r))
	{
		replies.POST("", m.addReply) // 添加回复
	}
	replyTotal := r.Group("/v1/reply_total", m.ctx.AuthMiddleware(r))
	{
...
func (m *Message) syncMessageReplies(c *wkhttp.Context) {
	...
	seq := c.Query("seq")
	resp, err := network.Get(fmt.Sprintf("%s/messages/%s/replies?seq=%s", m.ctx.GetConfig().WuKongIM.APIURL, messageID, seq), nil, nil)
	if err != nil {
...
func (m *Message) syncReplyTotal(c *wkhttp.Context) {
	...
	resp, err := network.Post(m.ctx.GetConfig().WuKongIM.APIURL+"/reply_total/sync", []byte(util.ToJson(req)), nil)
	if err != nil {
		m.Error("调用IM扩展服务失败！", zap.Error(err))
		c.ResponseError(errors.New("调用IM扩展服务失败！"))
		return
...
func (m *Message) addReply(c *wkhttp.Context) {
	...
	req.UID = loginUID
	req.Name = loginName
	resp, err := network.Post(m.ctx.GetConfig().WuKongIM.APIURL+"/replies", []byte(util.ToJson(req)), nil)
	if err != nil {
		m.Error("调用IM扩展服务失败！", zap.Error(err))
		c.ResponseError(errors.New("调用IM扩展服务失败！"))
		return
