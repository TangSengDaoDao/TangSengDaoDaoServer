...
func (m *managerDB) queryUserBlacklists(uid string) ([]*managerUserBlacklistModel, error) {
	...
	_, err := m.session.Select("`user`.name,`user`.uid,IFNULL(user_setting.updated_at,'') ").From("`user`").LeftJoin(`user_setting`, "user.uid=user_setting.to_uid and user_setting.blacklist=1").Where("`user_setting`.uid=?", uid).Load(&users)
	return users, err
}
